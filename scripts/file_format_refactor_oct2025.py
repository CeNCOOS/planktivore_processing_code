...
import cvtools_pvtr_simple as cvtools
...
def convert_tif_to_rawcolor(thedir,outputdir,subdirmins,maglev,settings):
    ...
    filedate=date_from_name_oct2025(thedir,maglev) # get the date from the file name
    ...
    #
            ...
            if fsize is not None:
                ...
                newname=re.sub(pattern, lambda m: m.group(1)+stringtime+m.group(3),ftif)
                ftif=newname
...
def convert_tar_to_rawcolor(thedir,outputdir,subdirmins,maglev,settings,bayer_pattern):
        try:
            ...
            for afile in numfiles:
                ...
                filetime=date_from_name_oct2025(tmpf,maglev) # get the date from the file name
                newfiledatestamp=int(filetime.timestamp()*1000000) # I think this should be the number to place in the file name.
                stringtime=str(newfiledatestamp)
                if filetime is None:
                    ...
                    nfout=tmpf[sslash+1:]
                    pattern=r'(low_mag_cam-)(\d+)(-)'
                    newname=re.sub(pattern, lambda m: m.group(1)+stringtime+m.group(3),nfout)
                    nfout=newname
                    idslash=nfout.rfind('\\')
...
#    fid=open('c:/users/flbahr/setup_process_planktivore_lowmag.json','r')
    ...
    ncore=plset[0]['ncore'] if 'ncore' in plset[0] else 2
