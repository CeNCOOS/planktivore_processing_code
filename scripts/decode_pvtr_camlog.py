...
class DualMagLog:
    ...
    def parse_lines(self):
        ...
        for line in self.lines:
            ...
            if parsed_line and "message" in parsed_line: # message is from the log_format line above and needs to be a section at the
                # end of the line, if this piece is missing then don't parse the line as we don't know what we have.
                try:
                    ...
                    print(e)
                    print(parsed_line['date_time_string'])
                    #pdb.set_trace()
                    continue
                ...
                try:
                    # $DMCTRL
                    #print(len(data_bits))
                    #if (data_bits[0].lstrip(' ')=="$DMCTRL"):
                    #		print(len(data_bits))
                    #if (data_bits[0].lstrip(' ')=="$DMCTRL"):
                    #		pdb.set_trace()
                    #if (data_bits[0].lstrip(' ') == "$DMCTRL" and len(data_bits) == 17): # original
                    if (data_bits[0].lstrip(' ') == "$DMCTRL" and len(data_bits) >= 17): #  change from ==20 to >=17
        ...
        # collate data into one matrix with camera string stringstamp
        try:
        ...
        #except:
        	...
        	for i in range(0,self.all_data.shape[0]):
        		...
        		if len(ind1) > 0:
        			...
        			avalue=self.all_data[i,0]/1000000
        			#pdb.set_trace()
        			print(datetime.datetime.fromtimestamp(avalue))
        			logger.info("Line: " + str(i) + ", could not find matching low_mag time: " + str(int(self.all_data[i,0]/1000000)))
        		...
        		if len(ind2) > 0:
        			...
        			avalue=self.all_data[i,0]/1000000
        			print(datetime.datetime.fromtimestamp(avalue))
        			logger.info("Line: " + str(i) + ", could not find high_mag matching time: " + str(int(self.all_data[i,0]/1000000)))
