...
import re
from date_from_name_oct2025 import date_from_name_oct2025
import pdb
# REMEMBER to CHANGE to either high_mag_cam or low_mag_cam consistently through code and json file.

if __name__=="__main__":
    json_file="c:/users/flbahr/documents/github/planktivore_processing_code/json_files/setup_process_planktivore_lowmag_oct2025parquet.json"
...
#    json_file=input("Enter the path to the JSON file with settings: ")
    ...
    for adir in nf:
        ...
        head,afile=os.path.split(thedir)
        parent_dir=os.path.basename(head)
        #
        ...
        if os.path.splitext(thedir)[1]=='.tar':
            ...
            # get partial names
                ...
                # need to fix names for new times
                for a in np.arange(0,len(part3)):
                    filetime=date_from_name_oct2025(part3[a],maglev)
                    newfiledatestamp=int(filedate.timestamp()*1000000) # I think this should be the number to place in the file name.
                    stringtime=str(newfiledatestamp)
                    pattern=r'(low_mag_cam-)(\d+)(-)'
                    newname=re.sub(pattern, lambda m: m.group(1)+stringtime+m.group(3),part3[a])
                    #pdb.set_trace()
            
                filenames.extend(part3)
            except Exception as e:
                print(f"Failure to process {thedir}")
                print(repr(e))
            #pdb.set_trace()
                #numfiles=np.arange(1,len(tnames))
        if os.path.splitext(thedir)[1]=='.png':
            part3=os.path.basename(thedir)
            name_we_want=parent_dir+'/'+part3
            #pdb.set_trace()
            filedate=date_from_name_oct2025(name_we_want,maglev)
            newfiledatestamp=int(filedate.timestamp()*1000000) # I think this should be the number to place in the file name.
            stringtime=str(newfiledatestamp)
            pattern=r'(low_mag_cam-)(\d+)(-)'
            newname=re.sub(pattern, lambda m: m.group(1)+stringtime+m.group(3),name_we_want)
            #pdb.set_trace()
            filenames.append(newname)
            #filenames.append(part3)
    df=pd.DataFrame(filenames)
    df.to_parquet('October_2025_Ahi_lowmag_level2.parquet',engine='pyarrow')
